...
\Y\B\4\D$\\{banner}$ \5
\.{"This\ is\ CTANGLE\ (Ve}\)\.{rsion\ 4.12.2)"}\par
...
\As9, 12, 14, 30, 37, 44, 49, 65, 70, 84, 91, 99\ETs101.
\U1.\fi
...
\PB{\\{tok\_mem}} }\par
\As26, 32, 38, 42, 45, 53, 57, 62, 66, 68\ETs82.
\U1.\fi
...
\M{25}The common lookup routine \PB{\\{id\_lookup}} refers to a separate
routine
\PB{\\{init\_node}} when the data structure grows.
...
\M{30}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\yskip\hang \PB{\\{byte\_field}} is the \PB{\\{tok\_mem}} location from which
the next
...
\PB{\\{stack}} array.  We call the current values \PB{\\{cur\_byte}}, \PB{%
\\{cur\_name}}, \PB{\\{cur\_repl}},
and \PB{\\{cur\_section}}.
...
\B\4\D$\\{cur\_state}$ \5
$\\{stack}[\\{stack\_size}+\T{1}{}$]\C{ \PB{\\{cur\_byte}}, \PB{\\{cur\_name}},
\PB{\\{cur\_repl}},   and \PB{\\{cur\_section}} }\par
\B\4\D$\\{cur\_byte}$ \5
...
\B\4\D$\\{cur\_end}$ \5
$(\\{cur\_repl}+\T{1})\MG{}$\\{tok\_start}\C{ current ending location in \PB{%
\\{tok\_mem}} }\par
\Y\B\4\X20:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{output\_state} ${}\\{stack}[\\{stack\_size}+\T{2}]{}$;\C{ info
...
\M{34}Similar settings are used for secondary output files.

\Y\B\4\X34:Initialize the secondary output\X${}\E{}$\6
$\\{stack\_ptr}\K\\{stack}+\T{1};{}$\6
${}\\{cur\_name}\K{*}\\{an\_output\_file};{}$\6
${}\\{cur\_repl}\K(\&{text\_pointer})\,\\{cur\_name}\MG\\{equiv};{}$\6
${}\\{cur\_byte}\K\\{cur\_repl}\MG\\{tok\_start}{}$;\par
\U50.\fi

\M{35}When the replacement text for name \PB{\|p} is to be inserted into the
output,
...
\&{bool} \\{flag})\C{ \PB{$\\{flag}\E\\{false}$} means we are in \PB{\\{output%
\_defs}} }\2\2\6
${}\{{}$\1\6
...
\M{37}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{void} \\{push\_level}(\&{name\_pointer});\6
\&{static} \&{void} \\{pop\_level}(\&{bool});\6
...
\&{static} \&{bool} \\{protect};\C{ should newline characters be quoted? }\par
\fi
...
\M{44}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\&{static} \&{name\_pointer} ${}{*}\\{cur\_out\_file},\39{*}\\{end\_output%
\_files},\39{*}\\{an\_output\_file};{}$\6
...
\X52:Output macro definitions if appropriate\X\6
...
\&{if} (\\{show\_progress})\5
${}\{{}$\1\6
${}\\{printf}(\\{cur\_out\_file}\E\\{end\_output\_files}\?\3{-1}\.{"\\nWriting\
the\ outpu}\)\.{t\ file\ (\%s):"}:\3{-1}\.{"\\nWriting\ the\ outpu}\)\.{t\
files:\ (\%s)"},\39\\{C\_file\_name});{}$\6
\\{update\_terminal}(\,);\6
\4${}\}{}$\2\6
\&{if} ${}(\\{text\_info}\MG\\{text\_link}\I\\{macro}){}$\1\5
\X51:Output material from \PB{\\{stack}}\X\2\6
...
\M{49}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{void} \\{phase\_two}(\&{void});\6
...
\4${}\}{}$\2\6
${}\\{cur\_line}\K\T{1};{}$\6
\X34:Initialize the secondary output\X\6
\X51:Output material from \PB{\\{stack}}\X\6
\4${}\}{}$\2\par
\U48.\fi

\M{51}\B\X51:Output material from \PB{\\{stack}}\X${}\E{}$\6
${}\{{}$\1\6
...
\M{52}If a \.{@h} was not encountered in the input,
...
\Y\B\4\X52:Output macro definitions if appropriate\X${}\E{}$\6
\&{if} ${}(\R\\{output\_defs\_seen}){}$\1\5
\\{output\_defs}(\,);\2\par
\U48.\fi

\M{53}\B\X20:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{bool} \\{output\_defs\_seen}${}\K\\{false}{}$;\par
\fi

\M{54}\B\D$\\{C\_printf}(\|c,\|a)$ \5
$\\{fprintf}(\\{C\_file},\39\|c,\39\|a{}$)\par
\B\4\D$\\{C\_putc}(\|c)$ \5
$\\{fputc}((\&{int})(\|c),\39\\{C\_file}{}$)\C{ isn't \CEE/ wonderfully
consistent? }\par
\Y\B\1\1\&{static} \&{void} \\{output\_defs}(\&{void})\2\2\6
${}\{{}$\1\6
\&{sixteen\_bits} \|a;\6
\&{eight\_bits} ${}{*}\\{macro\_end};{}$\7
${}\\{push\_level}(\NULL);{}$\6
...
${}\\{macro\_end}\K(\\{cur\_text}+\T{1})\MG\\{tok\_start}{}$;\C{ end of \PB{%
\\{macro}} replacement text }\6
...
\&{do}\5
${}\\{macro\_end}\MM;{}$\5
\&{while} ${}(\\{isspace}({*}\\{macro\_end})\W\\{plus\_plus}\I{*}\\{macro%
\_end}){}$;\C{ discard trailing whitespace; \PB{$\\{plus\_plus}\E\.{'\\v'}$} }\6
\&{while} ${}(\\{cur\_byte}\Z\\{macro\_end}){}$\5
${}\{{}$\1\6
${}\|a\K{*}\\{cur\_byte}\PP;{}$\6
\&{if} ${}(\\{out\_state}\E\\{verbatim}\W\|a\I\\{string}\W\|a\I\\{constant}\W%
\|a\I\.{'\\n'}){}$\1\5
...
${}\{{}$\1\6
\&{char} ${}{*}\|j{}$;\C{ pointer into \PB{\\{byte\_mem}} }\7
\4\\{restart}:\6
\&{switch} (\\{cur\_char})\5
${}\{{}$\1\6
\4\&{case} \.{'\\n'}:\6
...
\M{57}When an identifier is output to the \CEE/ file, characters in the range
128--255 (\PB{\T{\~200}}--\PB{\T{\~377}}) must be changed into something else,
so the \CEE/
...
\T{10}\par
\Y\B\4\X20:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{char} \\{translit}[\T{\~200}][\\{translit\_length}];\par
\fi

\M{58}\B\X21:Set initial values\X${}\mathrel+\E{}$\6
${}\{{}$\1\6
\&{int} \|i;\7
\&{for} ${}(\|i\K\T{0};{}$ ${}\|i<\T{\~200};{}$ ${}\|i\PP){}$\1\5
${}\\{snprintf}(\\{translit}[\|i],\39\\{translit\_length},\39\.{"X\%02X"},%
\39{}$(\&{unsigned} \&{int})(${}\T{\~200}+\|i));{}$\2\6
\4${}\}{}$\2\par
\fi
...
\&{if} ${}(\\{out\_state}\E\\{num\_or\_id}){}$\1\5
\\{C\_putc}(\.{'\ '});\2\6
\&{for} ${}(\|j\K(\\{cur\_val}+\\{name\_dir})\MG\\{byte\_start};{}$ ${}\|j<(%
\\{cur\_val}+\\{name\_dir}+\T{1})\MG\\{byte\_start};{}$ ${}\|j\PP){}$\1\6
\&{if} ${}(\\{ishigh}({*}\|j)){}$\1\5
...
\&{for} ${}(\|j\K(\\{cur\_val}+\\{name\_dir})\MG\\{byte\_start};{}$ ${}\|j<(%
\\{cur\_val}+\\{name\_dir}+\T{1})\MG\\{byte\_start};{}$ ${}\|j\PP){}$\5
${}\{{}$\1\6
\&{if} ${}({*}\|j\E\.{'\\\\'}\V{*}\|j\E\.{'"'}){}$\1\5
\\{C\_putc}(\.{'\\\\'});\2\6
${}\\{C\_putc}({*}\|j);{}$\6
...
\M{65}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{eight\_bits} \\{skip\_ahead}(\&{void});\6
\&{static} \&{bool} \\{skip\_comment}(\&{bool});\par
\fi
...
\Y\B\4\X20:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{bool} \\{comment\_continues}${}\K\\{false}{}$;\C{ are we scanning
a comment? }\par
\fi

\M{67}\B\1\1\&{static} \&{bool} \\{skip\_comment}(\C{ skips over comments }\6
\&{bool} \\{is\_long\_comment})\2\2\6
${}\{{}$\1\6
...
scanned }\6
\&{static} \&{bool} \\{no\_where};\C{ suppress \PB{\\{print\_where}}? }\par
\fi
...
input token }\2\2\6
${}\{{}$\1\6
\&{static} \&{bool} \\{preprocessing}${}\K\\{false};{}$\6
...
\M{70}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
${}\{{}$\1\6
\&{bool} \\{hex\_flag}${}\K\\{false}{}$;\C{ are we reading a hexadecimal
literal? }\7
...
\M{84}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\\{store\_id}(\|a);\6
\&{if} ${}({*}\\{buffer}\E\.{'\#'}\W\3{-1}((\\{id\_loc}-\\{id\_first}\E\T{5}\W%
\\{strncmp}(\.{"endif"},\39\\{id\_first},\39\T{5})\E\T{0})\V\3{-1}(\\{id\_loc}-%
\\{id\_first}\E\T{4}\W\\{strncmp}(\.{"else"},\39\\{id\_first},\39\T{4})\E\T{0})%
\V\3{-1}(\\{id\_loc}-\\{id\_first}\E\T{4}\W\\{strncmp}(\.{"elif"},\39\\{id%
\_first},\39\T{4})\E\T{0})){}$)\C{ Avoid preprocessor calamities }\1\6
${}\\{print\_where}\K\\{true};{}$\2\6
\&{break};\6
...
${}\{{}$\1\6
${}\PP\\{id\_first};{}$\6
${}\|c\K\T{16}*\|c+\\{toupper}((\&{int})\,{*}\\{id\_first})-(\&{int})\,\.{'A'}+%
\T{10};{}$\6
\4${}\}{}$\2\6
\&{break};\6
\4\&{case} \.{'\\\\'}:\5
...
\M{91}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\4${}\}{}$\2\6
\&{else} \&{if} ${}(\|p\MG\\{equiv}\E{}$(\&{void} ${}{*})\,\\{text\_info}){}$\1%
\5
...
\M{99}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\M{101}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\&{ptrdiff\_t})(\\{text\_ptr}-\\{text\_info}),\39(\&{long})\,\\{max%
\_texts});{}$\6
