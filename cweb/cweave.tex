...
\Y\B\4\D$\\{banner}$ \5
\.{"This\ is\ CWEAVE\ (Ver}\)\.{sion\ 4.12.2)"}\par
\Y\B\X5:Include files\X\6
\ATH\6
\X3:Common code for \.{CWEAVE} and \.{CTANGLE}\X\6
\X22:Typedef declarations\X\6
\X21:Private variables\X\6
\X4:Predeclaration of procedures\X\par
\fi
...
\As6, 7, 8, 10, 11, 13, 15\ETs16.
\U1.\fi

\M{4}The procedure that gets everything rolling:
\Y\B\4\X4:Predeclaration of procedures\X${}\E{}$\6
\&{extern} \&{void} \\{common\_init}(\&{void});\par
\As9, 12, 14, 25, 40, 45, 65, 69, 71, 83, 86, 90, 95, 98, 115, 118, 122, 181,
189, 194, 201, 210, 214, 228, 235, 244, 248, 258\ETs267.
\U1.\fi

\M{5}Interface to the standard \CEE/ library:

\Y\B\4\X5:Include files\X${}\E{}$\6
...
\M{6}Code related to the character set:
...
\M{7}Code related to input routines:
...
\M{8}Code related to file handling:
\Y\B\F\\{line}\5
...
\M{9}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
...
\M{10}Code related to section numbers:
\Y\B\4\X3:Common code for \.{CWEAVE} and \.{CTANGLE}\X${}\mathrel+\E{}$\6
...
\M{12}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
...
\M{13}Code related to error handling:
\Y\B\4\D$\\{spotless}$ \5
...
\M{14}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
...
\M{19}\.{CWEAVE} will use the \PB{\.{INT\_MAX}} limit in section
\PB{$\X231:Output the code for the beginning of a new section\X$} below.

\Y\B\4\X5:Include files\X${}\mathrel+\E{}$\6
...
\&{static} \&{bool} \\{change\_exists};\C{ has any section changed? }\par
\As23, 30, 37, 43, 46, 48, 67, 76, 81, 85, 106, 113, 119, 184, 208, 213, 229,
238, 250, 253, 255\ETs264.
\U1.\fi
...
\PB{\\{xmem}} }\6
\&{static} \&{sixteen\_bits} \\{xref\_switch}${},\39\\{section\_xref%
\_switch}{}$;\C{ either zero or \PB{\\{def\_flag}} }\par
\fi
...
\As31, 38, 61, 92, 107, 114, 155, 204, 209, 254\ETs256.
\U2.\fi
...
\PB{$\X121:Match a production at \PB{\\{pp}}, or increase \PB{\\{pp}} if there
is no match\X$} to avoid a lot of identifier looking up.
...
\Y\B\4\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
...
\&{sixteen\_bits} \|m${},\39\|n{}$;\C{ new and previous cross-reference value }%
\7
\&{if} (\\{no\_xref})\1\5
\&{return};\2\6
...
\M{32}Here are the two procedures needed to complete \PB{\\{id\_lookup}}:
\Y\B\1\1\&{bool} \\{names\_match}(\&{name\_pointer} \|p${},{}$\C{ points to the
proposed match }\6
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{eight\_bits} \\{ccode}[\T{256}]${}\K\{\\{ignore}\}{}$;\C{ meaning
of a char following \.{@} }\par
\fi
...
\N{1}{43}Inputting the next token.
...
\M{45}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{bool} \\{preprocessing}${}\K\\{false}{}$;\C{ are we scanning a
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{bool} \\{sharp\_include\_line}${}\K\\{false}{}$;\C{ are we
...
\M{65}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\M{69}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\4${}\}{}$\2\6
\X74:Store cross-references in the \9{t}\TEX/ part of a section\X\6
\X77:Store cross-references in the \9{d}definition part of a section\X\6
\X80:Store cross-references in the \9{c}\CEE/ part of a section\X\6
...
\&{else}\5
${}\{{}$\1\6
\&{bool} \\{is\_long\_comment}${}\K(\\{next\_control}\E\\{begin\_comment});{}$\7
...
\Y\B\4\X74:Store cross-references in the \9{t}\TEX/ part of a section\X${}\E{}$%
\6
\&{while} (\\{true})\5
${}\{{}$\1\6
...
${}\{{}$\1\6
\&{char} ${}{*}\\{src}\K\\{id\_first},\39{*}\\{dst}\K\\{id\_first};{}$\7
\&{while} ${}(\\{src}<\\{id\_loc}){}$\5
${}\{{}$\1\6
\&{if} ${}({*}\\{src}\E\.{'@'}){}$\1\5
${}\\{src}\PP;{}$\2\6
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{name\_pointer} \\{lhs}${},\39\\{rhs}{}$;\C{ pointers to \PB{%
\\{byte\_start}} for format identifiers }\6
...
\Y\B\4\X77:Store cross-references in the \9{d}definition part of a section\X${}%
\E{}$\6
...
\Y\B\4\X80:Store cross-references in the \9{c}\CEE/ part of a section\X${}\E{}$%
\6
...
pointer }\6
\&{static} \&{bool} \\{an\_output};\C{ did \PB{\\{file\_flag}} precede \PB{%
\\{cur\_xref}}? }\par
\fi
...
\M{83}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\B\4\D$\\{tex\_putc}(\|c)$ \5
$\\{fputc}(\|c,\39\\{active\_file}{}$)\par
\B\4\D$\\{tex\_puts}(\|c)$ \5
$\\{fputs}(\|c,\39\\{active\_file}{}$)\par
\Y\B\4\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{void} \\{flush\_buffer}(\&{char} ${}{*},\39\&{bool},\39%
\&{bool}){}$;\6
...
\_ptr}$} }\6
\&{bool} \\{per\_cent}${},\39{}$\&{bool} \\{carryover})\2\2\6
${}\{{}$\1\6
...
\Y\B\4\X89:Start \9{t}\TEX/ output\X${}\E{}$\6
...
\&{static} \&{void} ${}\\{out\_name}(\&{name\_pointer},\39\&{bool}){}$;\par
\fi

\M{96}\B\1\1\&{static} \&{void} \\{out\_section}(\&{sixteen\_bits} \|n)\2\2\6
${}\{{}$\1\6
\&{char} \|s[\T{6}];\7
${}\\{snprintf}(\|s,\39\T{6},\39\.{"\%d"},\39(\&{int})\,\|n);{}$\6
\\{out\_str}(\|s);\6
...
\&{bool} \\{quote\_xalpha})\2\2\6
${}\{{}$\1\6
\&{char} ${}{*}\|k{}$;\C{ pointer into \PB{\\{byte\_mem}} }\7
\\{out}(\.{'\{'});\6
\&{for} ${}(\|k\K\|p\MG\\{byte\_start};{}$ ${}\|k<(\|p+\T{1})\MG\\{byte%
\_start};{}$ ${}\|k\PP){}$\5
${}\{{}$\1\6
...
\&{static} \&{int} ${}\\{copy\_comment}(\&{bool},\39\&{int}){}$;\par
\fi
...
\&{bool} \\{is\_long\_comment}${},{}$\C{ is this a traditional \CEE/ comment? }%
\6
...
\+& \PB{\\{decl\_head}} \PB{\\{comma}} & \PB{\\{decl\_head}} \hfill $DC$\,\PB{%
\\{opt}}9 & \&{int} $x,{}$ \cr
...
\PB{\\{fn\_decl}}
\altt\PB{\\{int\_like}} \PB{\\{lbrace}} \PB{\\{decl}} \hfill $F=\\{din}\,D$
& \&{long} \\{time}(\,) $\{$\cr
...
statement\cr
\advance\midcol35pt
\+\dagit& \PB{\\{lbrace}} \altt\PB{\\{stmt}} \PB{\\{decl}} \PB{\\{function}} %
\PB{\\{rbrace}} & \PB{\\{stmt}} \hfill
...
$\PB{\\{force}}\,E\,\\{bsp}\,\PB{\\{noop}}\,\PB{\\{cancel}}\,S\,\\{force}$
& $\!\!$ \&{else} $\{x=0;\}$\cr
...
& $\!\!$ \&{if} ($x$) ${}\{\,y\,\}{}$\cr
\advance\midcol20pt
\+\dagit& \PB{\\{do\_like}} \PB{\\{stmt}} \PB{\\{else\_like}} \PB{\\{semi}} & %
\PB{\\{stmt}} \hfill
...
\hfill
$C$\,\\{din}\,\alt $C$ $E$ & \PB{\&{catch}\1\1${}(\,\ldots\,)$}\cr
...
\cr
\+& \PB{\\{insert}} \PB{\\{function}} & \PB{\\{function}} & \#\&{include}
before \\{main}\cr
...
\+& \PB{\\{delete\_like}} \PB{\\{lbrack}} \PB{\\{rbrack}} & \PB{\\{delete%
\_like}}\hfill $DL\.{\\,}R$
& \&{delete}[\,] \cr
...
\+& \PB{\\{exp}} \PB{\\{attr}} & \PB{\\{exp}} \hfill $E\.\ A$ & \&{enum} $\{x\
[[\ldots]]\}$ \cr
...
\_like}} \PB{\\{comma}} &
\PB{\\{langle}} \hfill $LS$\.\ \alt $E^{**}$ $I^{**}$ \unskip $C$\,\\{opt}9
...
\dag{\bf Notes}
\yskip
Rules 35, 117: The \PB{\\{exp}} must not be immediately followed by \PB{%
\\{lpar}}, \PB{\\{lbrack}},
\PB{\\{exp}}, or~\PB{\\{cast}}.
...
Rule 55: The second \PB{\\{force}} becomes \\{bsp} if \.{CWEAVE} has been
invoked
with the \.{-F} option.

Rule 69: The $\PB{\&{do}}\ldots\PB{\&{while}}$ loop is wrapped in \PB{%
\\{force}} if \.{CWEAVE} is invoked with the \.{-f} option.
...
\hbox{}\6{}\X252:Rest of \PB{\\{trans\_plus}} union\X\2\6
${}\}{}$ \\{trans\_plus};\2\6
${}\}{}$ \&{scrap};\6
\&{typedef} \&{scrap} ${}{*}\&{scrap\_pointer}{}$;\par
\fi

\M{113}\B\D$\\{max\_scraps}$ \5
...
\Y\B\4\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\8\#\&{ifdef} \.{DEBUG}\6
\&{static} \&{void} \\{print\_text}(\&{text\_pointer} \|p);\6
\8\#\&{endif}\par
\fi

\M{116}This function prints a token list for debugging;
it is not used in \PB{\\{main}} at this time.

\Y\B\8\#\&{ifdef} \.{DEBUG}\6
\1\1\&{static} \&{void} \\{print\_text}(\&{text\_pointer} \|p)\2\2\6
${}\{{}$\1\6
...
\B\4\D$\\{big\_app4}(\|a)$ \5
\\{big\_app3}(\|a);\5
$\\{big\_app1}(\|a+\T{3}{}$)\Y\par
\B\4\D$\\{big\_app1\_insert}(\|p,\|c)$ \5
\\{big\_app1}(\|p);\5
\\{big\_app}(\|c);\5
$\\{big\_app1}(\|p+\T{1}{}$)\par
\B\4\D$\\{big\_app1\_insert\_str}(\|p,\|s)$ \5
\\{big\_app1}(\|p);\5
\\{app\_str}(\|s);\5
$\\{big\_app1}(\|p+\T{1}{}$)\par
\B\4\D$\\{big\_app2\_insert}(\|p,\|c)$ \5
\\{big\_app2}(\|p);\5
\\{big\_app}(\|c);\5
$\\{big\_app2}(\|p+\T{2}{}$)\par
\Y\B\4\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{int} \\{cur\_mathness}${},\39\\{init\_mathness}{}$;\par
\fi
...
\Y\B\4\D$\\{begin\_math}()$ \6
\&{if} ${}(\\{cur\_mathness}\E\\{maybe\_math}){}$\1\5
${}\\{init\_mathness}\K\\{yes\_math};{}$\2\6
\&{else} \&{if} ${}(\\{cur\_mathness}\E\\{no\_math})$ \\{app\_str}(\.{"\$\{%
\}"})\par
\B\4\D$\\{end\_math}()$ \6
\&{if} ${}(\\{cur\_mathness}\E\\{maybe\_math}){}$\1\5
${}\\{init\_mathness}\K\\{no\_math};{}$\2\6
\&{else} \&{if} ${}(\\{cur\_mathness}\E\\{yes\_math})$ \\{app\_str}(\.{"\{\}%
\$"})\par
...
${}\{{}$\1\6
\&{xref\_pointer} \|q${}\K(\&{xref\_pointer})\,\|p\MG\\{xref}{}$;\C{ pointer to
cross-reference being examined }\6
...
${}\{{}$\1\6
${}\\{big\_app1\_insert\_str}(\\{pp},\39\.{"\\\\,"});{}$\6
...
\&{else} \&{if} ${}(\\{cat1}\E\\{int\_like}\V\\{cat1}\E\\{lbrace}\V\\{cat1}\E%
\\{decl}){}$\5
${}\{{}$\1\6
\\{big\_app}(\\{dindent});\6
...
\\{big\_app1}(\\{pp});\6
\\{big\_app}(\\{indent});\6
${}\\{big\_app}(\\{force\_first}\?\\{force}:\\{break\_space});{}$\6
${}\\{big\_app1}(\\{pp}+\T{1});{}$\6
\\{big\_app}(\\{force});\6
\\{big\_app}(\\{backup});\6
${}\\{big\_app1}(\\{pp}+\T{2});{}$\6
...
\B\4\D$\\{force\_first}$ \5
\\{flags}[\.{'F'}]\C{ should compound statement start on new line? }\par
\Y\B\4\X24:Set initial values\X${}\mathrel+\E{}$\6
$\\{force\_lines}\K\\{force\_first}\K\\{true}{}$;\par
\fi

\M{156}\B\X156:Cases for \PB{\\{stmt}}\X${}\E{}$\6
\&{if} ${}(\\{cat1}\E\\{stmt}\V\\{cat1}\E\\{decl}\V\\{cat1}\E\\{function}){}$\5
${}\{{}$\1\6
${}\\{big\_app1\_insert}(\\{pp},\39(\\{cat1}\E\\{decl}\V\\{cat1}\E\\{function})%
...
\&{if} ${}(\\{cat1}\E\\{if\_like}\V\\{cat1}\E\\{else\_like}\V\\{cat1}\E%
\\{define\_like}){}$\1\5
...
${}\\{squash}(\\{pp},\39\T{2},\39\\{function},\39\T{0},\39\T{83}){}$;\2\par
\U121.\fi
...
${}\\{big\_app2\_insert}(\\{pp},\39\.{'\ '});{}$\6
\&{if} ${}(\\{cat3}\E\\{comma}){}$\5
${}\{{}$\1\6
\\{app}(\\{opt});\6
\\{app}(\.{'9'});\6
${}\\{reduce}(\\{pp},\39\T{4},\39\\{langle},\39\T{0},\39\T{153});{}$\6
\4${}\}{}$\2\6
\&{else}\1\5
...
\4${}\}{}$\2\6
\&{else} \&{if} ${}(\\{cat1}\E\\{comma}){}$\1\5
${}\\{squash}(\\{pp},\39\T{2},\39\\{typedef\_like},\39\T{0},\39\T{118});{}$\2\6
\&{else} \&{if} ${}(\\{cat1}\E\\{semi}){}$\1\5
...
\&{if} ${}(\\{cat1}\E\\{lbrack}\W\\{cat2}\E\\{rbrack}){}$\5
${}\{{}$\1\6
\\{big\_app1}(\\{pp});\6
${}\\{big\_app1\_insert\_str}(\\{pp}+\T{1},\39\.{"\\\\,"});{}$\6
...
productions,
which takes advantage of the simplifications that occur when \PB{$\|k\E\T{0}$}
or \PB{$\|k\E\T{1}$}.
...
\&{short} \|n)\2\2\6
${}\{{}$\1\6
\&{scrap\_pointer} \|i;\C{ pointer into scrap memory }\7
${}\|j\MG\\{cat}\K\|c;{}$\6
\&{if} ${}(\|k>\T{0}){}$\5
${}\{{}$\1\6
...
\&{if} ${}(\|k>\T{1}){}$\5
${}\{{}$\1\6
\&{for} ${}(\|i\K\|j+\|k;{}$ ${}\|i\Z\\{lo\_ptr};{}$ ${}\|i\PP){}$\1\5
${}{*}(\|i-\|k+\T{1})\K{*}\|i;{}$\2\6
${}\\{lo\_ptr}\K\\{lo\_ptr}-\|k+\T{1};{}$\6
\4${}\}{}$\2\6
${}\\{pp}\K(\\{pp}+\|d<\\{scrap\_base}\?\\{scrap\_base}:\\{pp}+\|d);{}$\6
\X185:Print a snapshot of the scrap list if debugging\X\6
...
\M{184}If \.{CWEAVE} is being run in debugging mode, the production numbers and
...
\M{185}\B\X185:Print a snapshot of the scrap list if debugging\X${}\E{}$\6
\&{if} ${}(\\{tracing}\E\\{fully}){}$\5
${}\{{}$\1\6
${}\\{printf}(\.{"\\n\%d:"},\39\|n);{}$\6
\&{for} ${}(\|i\K\\{scrap\_base};{}$ ${}\|i\Z\\{lo\_ptr};{}$ ${}\|i\PP){}$\5
${}\{{}$\1\6
${}\\{putchar}(\|i\E\\{pp}\?\.{'*'}:\.{'\ '});{}$\6
\&{if} ${}(\|i\MG\\{mathness}\MOD\T{4}\E\\{yes\_math}){}$\1\5
\\{putchar}(\.{'+'});\2\6
\&{else} \&{if} ${}(\|i\MG\\{mathness}\MOD\T{4}\E\\{no\_math}){}$\1\5
\\{putchar}(\.{'-'});\2\6
${}\\{print\_cat}(\|i\MG\\{cat});{}$\6
\&{if} ${}(\|i\MG\\{mathness}/\T{4}\E\\{yes\_math}){}$\1\5
\\{putchar}(\.{'+'});\2\6
\&{else} \&{if} ${}(\|i\MG\\{mathness}/\T{4}\E\\{no\_math}){}$\1\5
\\{putchar}(\.{'-'});\2\6
\4${}\}{}$\2\6
...
\M{186}And here now is the code that applies productions as long as possible.
...
\T{10}\par
\Y\B\4\X186:Reduce the scraps using the productions until no more rules apply%
\X${}\E{}$\6
\&{while} (\\{true})\5
${}\{{}$\1\6
\X187:Make sure the entries \PB{\\{pp}} through \PB{$\\{pp}+\T{3}$} of \PB{%
\\{cat}} are defined\X\6
...
\M{187}If we get to the end of the scrap list, category codes equal to zero are
stored, since zero does not match anything in a production.

\Y\B\4\X187:Make sure the entries \PB{\\{pp}} through \PB{$\\{pp}+\T{3}$} of %
\PB{\\{cat}} are defined\X${}\E{}$\6
\&{if} ${}(\\{lo\_ptr}<\\{pp}+\T{3}){}$\5
${}\{{}$\1\6
\&{while} ${}(\\{hi\_ptr}\Z\\{scrap\_ptr}\W\\{lo\_ptr}\I\\{pp}+\T{3}){}$\1\5
${}{*}(\PP\\{lo\_ptr})\K{*}(\\{hi\_ptr}\PP);{}$\2\6
\&{for} ${}(\|j\K\\{lo\_ptr}+\T{1};{}$ ${}\|j\Z\\{pp}+\T{3};{}$ ${}\|j\PP){}$\1%
\5
${}\|j\MG\\{cat}\K\T{0};{}$\2\6
\4${}\}{}$\2\par
\U186.\fi
...
\X186:Reduce the scraps using the productions until no more rules apply\X\6
\X190:Combine the irreducible scraps that remain\X\6
\4${}\}{}$\2\par
\fi

\M{189}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\M{194}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\&{case} \\{constant}:\5
\&{case} \\{verbatim}:\5
\X199:Append a \9{s}string or constant\X\5
\&{break};\6
\4\&{case} \\{identifier}:\5
\\{app\_cur\_id}(\\{true});\5
\&{break};\6
\4\&{case} ${}\TeXxstring{}$:\5
\X200:Append a \9{t}\TEX/ string, without forming a scrap\X\5
\&{break};\6
\4\&{case} \.{'/'}:\5
\&{case} \.{'.'}:\5
\\{app}(\\{next\_control});\5
...
\Y\B\4\X199:Append a \9{s}string or constant\X${}\E{}$\6
${}\{{}$\5
...
\Y\B\4\X200:Append a \9{t}\TEX/ string, without forming a scrap\X${}\E{}$\6
\\{app\_str}(\.{"\\\\hbox\{"});\6
\&{while} ${}(\\{id\_first}<\\{id\_loc}){}$\5
${}\{{}$\1\6
\&{if} ${}((\&{eight\_bits})({*}\\{id\_first})>\T{\~177}){}$\1\5
...
\Y\B\4\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{void} \\{app\_cur\_id}(\&{bool});\6
\&{static} \&{text\_pointer} \\{C\_translate}(\&{void});\6
\&{static} \&{void} \\{outer\_parse}(\&{void});\par
\fi

\M{202}\B\1\1\&{static} \&{void} \\{app\_cur\_id}(\&{bool} \\{scrapping})\C{
...
\&{text\_pointer} \|p${},\39\|q{}$;\C{ partial comments }\7
\&{while} ${}(\\{next\_control}<\\{format\_code}){}$\1\6
\&{if} ${}(\\{next\_control}\I\\{begin\_comment}\W\\{next\_control}\I\\{begin%
\_short\_comment}){}$\1\5
\\{C\_parse}(\\{ignore});\2\6
...
\Y\B\4\X22:Typedef declarations\X${}\mathrel+\E{}$\6
\&{typedef} \&{enum} ${}\{{}$\1\6
${}\\{inner},{}$\C{ value of \PB{\&{mode}} for \CEE/ texts within \TEX/ texts }%
\6
\\{outer}\C{ value of \PB{\&{mode}} for \CEE/ texts in sections }\2\6
${}\}{}$ \&{mode};\6
\&{typedef} \&{struct} ${}\{{}$\1\6
...
\&{mode} \\{mode\_field};\C{ interpretation of control tokens }\2\6
${}\}{}$ \&{output\_state};\6
...
\B\4\D$\\{cur\_state}$ \5
\\{stack}[\\{stack\_size}]\C{ \PB{\\{cur\_end}}, \PB{\\{cur\_tok}}, \PB{\\{cur%
\_mode}} }\par
\B\4\D$\\{cur\_end}$ \5
...
\M{210}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\6
\&{static} \&{void} \\{push\_level}(\&{text\_pointer});\par
\fi

\M{211}To insert token-list \PB{\|p} into the output, the \PB{\\{push\_level}}
subroutine
...
\\{overflow}(\.{"stack"});\2\6
\&{if} ${}(\\{stack\_ptr}>\\{stack}{}$)\C{ save current state }\1\6
${}{*}\\{stack\_ptr}\K\\{cur\_state};{}$\2\6
${}\\{stack\_ptr}\PP;{}$\6
...
called when \PB{$\\{stack\_ptr}\E\T{1}$}. It is so simple, we declare it as a
macro:

\Y\B\4\D$\\{pop\_level}()$ \5
$\\{cur\_state}\K{*}(\MM\\{stack\_ptr}{}$)\par
\fi
...
section names }\6
\&{char} ${}{*}\|k,\39{*}\\{k\_limit}{}$;\C{ indices into \PB{\\{scratch}} }\6
...
\&{char} ${}{*}\\{save\_loc},\39{*}\\{save\_limit}{}$;\C{ \PB{\\{loc}} and \PB{%
\\{limit}} to be restored }\6
\&{name\_pointer} \\{cur\_section\_name};\C{ name of section being output }\6
\&{bool} \\{save\_mode};\C{ value of \PB{\\{cur\_mode}} before a sequence of
breaks }\6
\&{bool} \\{dindent\_pending}${}\K\\{false}{}$;\C{ should a \PB{\\{dindent}} be
output? }\7
...
\&{if} (\\{an\_output})\1\5
${}\\{cur\_mode}\E\\{inner}\?\\{out\_str}(\.{"\}"}):\\{out\_str}(\.{"\\\\,%
\}"});{}$\2\6
\\{out\_str}(\.{"\\\\X"});\par
\U217.\fi
...
\&{if} ${}({*}\|k\PP\I\.{'@'}){}$\5
${}\{{}$\1\6
${}\\{printf}(\.{"\%s"},\39\.{"\\n!\ Illegal\ control}\)\.{\ code\ in\ section\
nam}\)\.{e:\ <"});{}$\6
...
\&{if} ${}(\|k\G\\{k\_limit}){}$\5
${}\{{}$\1\6
${}\\{printf}(\.{"\%s"},\39\.{"\\n!\ C\ text\ in\ secti}\)\.{on\ name\ didn't\
end:\ }\)\.{<"});{}$\6
\\{print\_section\_name}(\\{cur\_section\_name});\6
\\{printf}(\.{">\ "});\6
\\{mark\_error}(\,);\6
...
\&{while} ${}(\R\\{input\_has\_ended}){}$\1\5
\X230:Translate the \9{c}current section\X\2\6
\4${}\}{}$\2\par
\fi

\M{228}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\&{static} \&{bool} \\{space\_checked};\C{ have we done \PB{\\{emit\_space\_if%
\_needed}}? }\6
\&{static} \&{bool} \\{format\_visible};\C{ should the next format declaration
be output? }\6
\&{static} \&{bool} \\{doing\_format}${}\K\\{false}{}$;\C{ are we outputting a
format declaration? }\6
\&{static} \&{bool} \\{group\_found}${}\K\\{false}{}$;\C{ has a starred section
occurred? }\par
\fi

\M{230}\B\X230:Translate the \9{c}current section\X${}\E{}$\6
${}\{{}$\1\6
${}\\{section\_count}\PP;{}$\6
\X231:Output the code for the beginning of a new section\X\6
\\{save\_position}(\,);\6
\X232:Translate the \9{t}\TEX/ part of the current section\X\6
\X233:Translate the \9{d}definition part of the current section\X\6
\X239:Translate the \9{c}\CEE/ part of the current section\X\6
...
\\{loc}\PP){}$\1\6
\&{if} ${}(\\{sec\_depth}<\.{INT\_MAX}/\T{10}){}$\1\5
${}\\{sec\_depth}\K\\{sec\_depth}*\T{10}+({*}\\{loc})-\.{'0'};{}$\2\2\6
\4${}\}{}$\2\6
\&{while} ${}({*}\\{loc}\E\.{'\ '}){}$\1\5
...
${}\{{}$\5
\1\&{char} \|s[\T{32}];\5
${}\\{snprintf}(\|s,\39\T{32},\39\.{"\{\%d\}"},\39\\{sec\_depth}+\T{1}){}$;\5
\\{out\_str}(\|s);\5
${}\}{}$\2\6
\&{if} (\\{show\_progress})\5
${}\{{}$\1\6
...
\Y\B\4\X232:Translate the \9{t}\TEX/ part of the current section\X${}\E{}$\6
\&{do}\5
...
\&{if} ${}(\\{next\_control}\E\\{verbatim}){}$\1\5
\\{err\_print}(\.{"!\ Verbatim\ string\ s}\)\.{hould\ be\ in\ C\ text\ o}\)%
\.{nly"});\2\6
\&{break};\6
\4\&{case} \\{thin\_space}:\5
\&{case} \\{math\_break}:\5
...
\Y\B\4\X233:Translate the \9{d}definition part of the current section\X${}\E{}$%
\6
...
\X236:Start \9{a}a macro definition\X\2\6
\&{else}\1\5
\X237:Start \9{a}a format definition\X\2\6
\\{outer\_parse}(\,);\6
...
part }\6
\&{bool} \\{visible})\C{ \PB{\\{true}} if we should produce \TeX\ output }\2\2\6
${}\{{}$\1\6
...
\M{235}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
\&{static} \&{void} \\{finish\_C}(\&{bool});\par
\fi
...
\Y\B\4\X236:Start \9{a}a macro definition\X${}\E{}$\6
${}\{{}$\1\6
...
\M{237}\B\X237:Start \9{a}a format definition\X${}\E{}$\6
${}\{{}$\1\6
...
\M{239}\B\X239:Translate the \9{c}\CEE/ part of the current section\X${}\E{}$\6
...
\M{244}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
${}\{{}$\1\6
${}\\{phase}\K\T{3};{}$\6
\\{finish\_line}(\,);\C{ the bulk of \PB{\\{tex\_file}} has been written }\6
\&{if} (\\{no\_xref})\1\5
\\{out\_str}(\.{"\\\\end"});\2\6
\&{else}\5
${}\{{}$\1\6
...
\&{if} (\\{change\_exists})\5
${}\{{}$\1\6
\X249:Tell about changed sections\X\6
\\{finish\_line}(\,);\6
${}\\{flush\_buffer}(\\{out\_buf},\39\\{false},\39\\{false}){}$;\C{ insert a
blank line, it looks nice }\6
\4${}\}{}$\2\6
...
\X251:Do the first pass of sorting\X\6
\X259:Sort and output the index\X\6
\\{finish\_line}(\,);\6
\\{fclose}(\\{active\_file});\C{ finished with \PB{\\{idx\_file}} }\7
...
file }\6
\X268:Output all the section names\X\6
\\{finish\_line}(\,);\6
\\{fclose}(\\{active\_file});\C{ finished with \PB{\\{scn\_file}} }\7
${}\\{active\_file}\K\\{tex\_file}{}$;\C{ switch back to \PB{\\{tex\_file}} for
the last time }\6
\&{if} (\\{group\_found})\1\5
\\{out\_str}(\.{"\\\\con"});\5
...
\M{248}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
...
\Y\B\4\X249:Tell about changed sections\X${}\E{}$\C{ remember that the index is
already marked as changed }\6
\&{sixteen\_bits} \\{k\_section}${}\K\T{0}{}$;\C{ runs through the sections }\7
...
\M{250}A left-to-right radix sorting method is used, since this makes it easy
to
...
\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{name\_pointer} \\{bucket}[\T{256}]${}\K\{\NULL\};{}$\6
...
\M{251}To begin the sorting, we go through all the hash lists and put each
entry
having a nonempty cross-reference list into the proper bucket.

\Y\B\4\X251:Do the first pass of sorting\X${}\E{}$\6
\&{for} ${}(\\{hash\_ptr}\K\\{hash};{}$ ${}\\{hash\_ptr}\Z\\{hash\_end};{}$ ${}%
\\{hash\_ptr}\PP){}$\5
${}\{{}$\1\6
${}\\{next\_name}\K{*}\\{hash\_ptr};{}$\6
\&{while} (\\{next\_name})\5
${}\{{}$\1\6
...
${}\{{}$\1\6
\&{int} \|c${}\K(\\{cur\_name}\MG\\{byte\_start})[\T{0}];{}$\7
\&{if} (\\{xisupper}(\|c))\1\5
${}\|c\K\\{tolower}(\|c);{}$\2\6
...
\M{252}During the sorting phase we shall use the \PB{\\{cat}} and \PB{%
\\{trans}} arrays from
...
\Y\B\4\X252:Rest of \PB{\\{trans\_plus}} union\X${}\E{}$\6
...
\M{254}\B\X24:Set initial values\X${}\mathrel+\E{}$\6
$\\{max\_sort\_ptr}\K\\{scrap\_info}{}$;\par
\fi

\M{255}The desired alphabetic order is specified by the \PB{\\{collate}} array;
namely,
...
\M{256}We use the order $\hbox{null}<\.\ <\hbox{other characters}<{}$\.\_${}<
...
\Y\B\4\X24:Set initial values\X${}\mathrel+\E{}$\6
$\\{collate}[\T{0}]\K\T{0};{}$\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{1},\39\.{"\ \\1\\2\\3\\4\\5\\6\\7%
\\10\\}\)\.{11\\12\\13\\14\\15\\16\\17}\)\.{"},\39\T{16}){}$;\C{ 16 characters
+ 1 = 17 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{17},\39\.{"\\20\\21\\22\\23\\24%
\\25\\}\)\.{26\\27\\30\\31\\32\\33\\34}\)\.{\\35\\36\\37"},\39\T{16}){}$;\C{ 16
characters + 17 = 33 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{33},\39\.{"!\\42\#\$%
\%\&'()*+,-./:;}\)\.{<=>?@[\\\\]\^`\{|\}\~\_"},\39\T{32}){}$;\C{ 32 characters
+ 33 = 65 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{65},\39\.{"abcdefghijklmnopqrs}\)%
\.{tuvwxyz0123456789"},\39\T{36}){}$;\C{ (26 + 10) characters + 65 = 101 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{101},\39\.{"\\200\\201\\202\\203%
\\20}\)\.{4\\205\\206\\207\\210\\21}\)\.{1\\212\\213\\214\\215\\21}\)\.{6%
\\217"},\39\T{16}){}$;\C{ 16 characters + 101 = 117 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{117},\39\.{"\\220\\221\\222\\223%
\\22}\)\.{4\\225\\226\\227\\230\\23}\)\.{1\\232\\233\\234\\235\\23}\)\.{6%
\\237"},\39\T{16}){}$;\C{ 16 characters + 117 = 133 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{133},\39\.{"\\240\\241\\242\\243%
\\24}\)\.{4\\245\\246\\247\\250\\25}\)\.{1\\252\\253\\254\\255\\25}\)\.{6%
\\257"},\39\T{16}){}$;\C{ 16 characters + 133 = 149 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{149},\39\.{"\\260\\261\\262\\263%
\\26}\)\.{4\\265\\266\\267\\270\\27}\)\.{1\\272\\273\\274\\275\\27}\)\.{6%
\\277"},\39\T{16}){}$;\C{ 16 characters + 149 = 165 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{165},\39\.{"\\300\\301\\302\\303%
\\30}\)\.{4\\305\\306\\307\\310\\31}\)\.{1\\312\\313\\314\\315\\31}\)\.{6%
\\317"},\39\T{16}){}$;\C{ 16 characters + 165 = 181 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{181},\39\.{"\\320\\321\\322\\323%
\\32}\)\.{4\\325\\326\\327\\330\\33}\)\.{1\\332\\333\\334\\335\\33}\)\.{6%
\\337"},\39\T{16}){}$;\C{ 16 characters + 181 = 197 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{197},\39\.{"\\340\\341\\342\\343%
\\34}\)\.{4\\345\\346\\347\\350\\35}\)\.{1\\352\\353\\354\\355\\35}\)\.{6%
\\357"},\39\T{16}){}$;\C{ 16 characters + 197 = 213 }\6
\\{memcpy}((\&{char} ${}{*})\,\\{collate}+\T{213},\39\.{"\\360\\361\\362\\363%
\\36}\)\.{4\\365\\366\\367\\370\\37}\)\.{1\\372\\373\\374\\375\\37}\)\.{6%
\\377"},\39\T{16}){}$;\C{ 16 characters + 213 = 229 }\par
\fi

\M{257}Procedure \PB{\\{unbucket}} goes through the buckets and adds nonempty
lists
...
\M{258}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
\&{static} \&{void} \\{unbucket}(\&{eight\_bits});\par
\fi

\M{259}\B\X259:Sort and output the index\X${}\E{}$\6
...
\X261:Output index entries for the list at \PB{\\{sort\_ptr}}\X\2\6
\&{else}\1\5
\X260:Split the list at \PB{\\{sort\_ptr}} into further lists\X\2\6
\4${}\}{}$\2\par
\U247.\fi

\M{260}\B\X260:Split the list at \PB{\\{sort\_ptr}} into further lists\X${}%
\E{}$\6
${}\{{}$\1\6
\&{int} \|c;\7
...
\&{else} \&{if} ${}(\\{xisupper}(\|c\K{*}\\{cur\_byte})){}$\1\5
${}\|c\K\\{tolower}(\|c);{}$\2\6
${}\\{blink}[\\{cur\_name}-\\{name\_dir}]\K\\{bucket}[\|c];{}$\6
${}\\{bucket}[\|c]\K\\{cur\_name};{}$\6
\4${}\}{}$\2\5
...
\M{261}\B\X261:Output index entries for the list at \PB{\\{sort\_ptr}}\X${}%
\E{}$\6
${}\{{}$\1\6
...
${}\{{}$\1\6
\\{out\_str}(\.{"\\\\I"});\6
\X262:Output the name at \PB{\\{cur\_name}}\X\6
\X263:Output the cross-references at \PB{\\{cur\_name}}\X\6
...
\M{262}\B\X262:Output the name at \PB{\\{cur\_name}}\X${}\E{}$\6
\&{switch} ${}(\\{cur\_name}\MG\\{ilk}){}$\5
${}\{{}$\5
\1\&{char} ${}{*}\|p{}$;\C{ index into \PB{\\{byte\_mem}} }\5
...
\&{else}\5
${}\{{}$\5
\1\&{bool} \\{all\_caps}${}\K\\{true}{}$;\5
\hbox{}\6{}\&{for} ${}(\|p\K\\{cur\_name}\MG\\{byte\_start};{}$ ${}\|p<(\\{cur%
\_name}+\T{1})\MG\\{byte\_start};{}$ ${}\|p\PP){}$\1\6
\&{if} ${}(\\{xislower}({*}\|p)){}$\5
${}\{{}$\C{ not entirely uppercase }\1\6
${}\\{all\_caps}\K\\{false};{}$\6
\&{break};\6
\4${}\}{}$\2\2\6
${}\\{out\_str}(\\{all\_caps}\?\.{"\\\\."}:\.{"\\\\\\\\"});{}$\6
\4${}\}{}$\2\6
\&{break};\6
\4\&{case} \\{wildcard}:\5
\\{out\_str}(\.{"\\\\9"});\5
...
\M{263}Section numbers that are to be underlined are enclosed in
`\.{\\[}$\,\ldots\,$\.]'.

\Y\B\4\X263:Output the cross-references at \PB{\\{cur\_name}}\X${}\E{}$\6
\X265:Invert the cross-reference list at \PB{\\{cur\_name}}, making \PB{\\{cur%
\_xref}} the head\X\6
\&{do}\5
${}\{{}$\1\6
\\{out\_str}(\.{",\ "});\6
...
\M{264}List inversion is best thought of as popping elements off one stack and
pushing them onto another. In this case \PB{\\{cur\_xref}} will be the head of
the stack that we push things onto.

\Y\B\4\X21:Private variables\X${}\mathrel+\E{}$\6
\&{static} \&{xref\_pointer} \\{next\_xref}${},\39\\{this\_xref}{}$;\C{ pointer
variables for rearranging a list }\par
\fi

\M{265}\B\X265:Invert the cross-reference list at \PB{\\{cur\_name}}, making %
...
\M{266}The following recursive procedure walks through the tree of section
names and
prints them.
...
\M{267}\B\X4:Predeclaration of procedures\X${}\mathrel+\E{}$\5
\&{static} \&{void} \\{section\_print}(\&{name\_pointer});\par
\fi

\M{268}\B\X268:Output all the section names\X${}\E{}$\6
\\{section\_print}(\\{root});\par
\U247.\fi

\M{269}Because on some systems the difference between two pointers is a \PB{%
\&{ptrdiff\_t}}
